if ( i_conv_desc->unroll_kh == 0 ) {
  /* open KH loop, kj */
  libxs_generator_convolution_header_kh_loop(  io_generated_code, &l_loop_label_tracker,
...
for ( l_kh = 0; l_kh < l_kh_trips; l_kh++) {
  if ( i_conv_desc->unroll_kw == 0 ) {
    /* open KW loop, ki */
    libxs_generator_convolution_header_kw_loop(  io_generated_code, &l_loop_label_tracker,
  ...
  if (l_kw_trips == 1) {
    ...
    if ( (i_conv_desc->ofw_rb == 7)  && ( i_conv_desc->ofh_rb == 4 || i_conv_desc->ofh_rb == 3 ) ) {
      /* Call with 3 or 4 rows  */
      libxs_generator_convolution_forward_avx512_ifmloop_qfma_x_rows( io_generated_code, &l_gp_reg_mapping, &l_conv_kernel_config, i_conv_desc, l_kw_trips, i_conv_desc->ofh_rb);
    } else {
      libxs_generator_convolution_forward_avx512_ifmloop(  io_generated_code,
          &l_gp_reg_mapping,
    ...
    libxs_x86_instruction_alu_imm(  io_generated_code,
    ...
    if ( (i_conv_desc->prefetch & LIBXS_CONVOLUTION_PREFETCH_WEIGHT_L2) == LIBXS_CONVOLUTION_PREFETCH_WEIGHT_L2 ) {
      libxs_x86_instruction_alu_imm(  io_generated_code,
          l_conv_kernel_config.alu_add_instruction,
    ...
    if ( (i_conv_desc->prefetch & LIBXS_CONVOLUTION_PREFETCH_INPUT_L1) == LIBXS_CONVOLUTION_PREFETCH_INPUT_L1 ) {
      libxs_x86_instruction_alu_imm( io_generated_code,
          l_conv_kernel_config.alu_add_instruction,
  ...
  if ( i_conv_desc->unroll_kw == 0 ) {
    /* close KW loop, ki */
    libxs_generator_convolution_footer_kw_loop(  io_generated_code, &l_loop_label_tracker,
...
if ( i_conv_desc->unroll_kh == 0 ) {
  /* close KH loop, kj */
  libxs_generator_convolution_footer_kh_loop(  io_generated_code, &l_loop_label_tracker,
