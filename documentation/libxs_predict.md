...
fingerprint-guided polynomial interpolation. Supports
incremental training, model persistence, and thread-safe
evaluation.
...
single-threaded). Returns EXIT_SUCCESS or EXIT_FAILURE.
...
Retrieve model statistics. Any output pointer may be NULL.
...
the separator. Column indices are 0-based. Rows with
...
reliable[j] is non-zero when the fingerprint indicates
smooth (predictable) variation. cluster is the assigned
cluster index (-1 if blended).
