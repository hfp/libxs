...
bazel build --verbose_failures -c opt --cxxopt=-D_GLIBCXX_USE_CXX11_ABI=0 \
  --copt=-O2 --copt=-fopenmp-simd --copt=-DLIBXS_OPENMP_SIMD \
...
If specific target flags are desired (instead of `-mfma -mavx2`), please refer to the [TensorFlow with LIBXS](tensorflow.md#specific-codepath) document. This document can be referred for more details in general.
