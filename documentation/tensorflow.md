...
This document is an early recipe for building and running TensorFlow with LIBXS. The amount of covered code paths as well as the performance of these code paths will be improved as the integration progresses. This means that executing TensorFlow for any real workload (or benchmark) beside of the cases shown below **may not use LIBXS at all** (same is/remains true for any system without Intel&#160;AVX2 instruction set extension). Relying on the master revision of TensorFlow is perfectly fine since the integration work is merged on a frequent basis. However, to capture the latest revision of the integration one may rely on:

```bash
git clone https://github.com/benoitsteiner/tensorflow-xsmm.git
wget https://github.com/hfp/libxs/archive/master.zip
sha256sum libxs-master.zip
```

To try LIBXS's master revision, the file `tensorflow/workspace.bzl` can be adjusted by using the SHA256 sum from above:

```Python
  native.new_http_archive(
      name = "libxs_archive",
      urls = [
          "https://github.com/hfp/libxs/archive/master.zip",
      ],
      sha256 = "<sha256sum libxs-master.zip>",
      strip_prefix = "libxs-master",
      build_file = str(Label("//third_party:libxs.BUILD")),
  )
```

<a name="non-default-compiler"></a>LIBXS does not impose to build for a specific code path, and always exploits the most suitable instruction set extension for JIT-enabled code paths. However, LIBXS may also use non-JIT code paths which are CPUID-dispatched when the static code path has lower capabilities. This only works when using GCC&#160;4.9 (or later) or the Intel Compiler. If TensorFlow does not match the highest possible CPU target (march=native), a performance penalty is possible. With recent Bazel versions, a non-default compiler can be source'd just normally i.e., added the the PATH environment. Beside of the regular TF-prerequisites, nothing else is needed to use TensorFlow with LIBXS.
...
**NOTE**: TensorFlow may run into issues, and one may temporarily apply `--copt=-mfma --copt=-mavx2` (even if Intel&#160;AVX-512 extensions are available). There are at least two issues: (1)&#160;there can be NaNs e.g., printed when training a network regardless of the GCC-version, or (2)&#160;TensorFlow *without* LIBXS may crash in TF's implementation of GEMM.

For AVX-512 in general, GCC&#160;5.x (or higher) should be used (see section [Non-default Compiler](#non-default-compiler)). LIBXS supports Intel&#160;AVX2 as the baseline code path for all JIT-generated DNN-code (SMM domain also supports AVX). For Intel&#160;AVX-512 (on top of AVX2), the foundational instructions are sufficient in many cases, but for the sparse domain the Core-flavor is a prerequisite ("Skylake server" or SKX), and VNNI/QFMA instructions are honored on Intel Xeon&#160;Phi code-named "Knights Mill" (KNM).
...
```bash
bazel build --copt=-O2 --copt=-fopenmp-simd --copt=-DLIBXS_OPENMP_SIMD --linkopt=-pthread \
...
bazel run --copt=-O2 --copt=-fopenmp-simd --copt=-DLIBXS_OPENMP_SIMD --linkopt=-pthread \
  --define tensorflow_xsmm=1 --define tensorflow_xsmm_convolutions=1 --define tensorflow_xsmm_backward_convolutions=1 \
  <line-of-target-flags-from-above> \
