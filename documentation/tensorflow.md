...
Beside of the regular prerequisites, nothing else is needed to use TensorFlow with LIBXS. However, at least in some cases (NFS-hosted directory, etc.) it may help to adjust the `configure` script (change `bazel clean --expunge` into `bazel clean --expunge_async`).
