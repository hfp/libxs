...
# Meta Image File I/O
Loading and storing data (I/O) is normally out of LIBXS's scope. However, comparing results (correctness) or writing files for visual inspection is clearly desired. This is particularly useful with images when processed by the DNN domain. The MHD function domain provides support for the Meta Image File format (MHD). Tools such as ITK-SNAP or ParaView can be used to inspect, compare, and modify images even beyond two-dimensional images.

Writing an image is per `libxs_mhd_write`, and loading an image split in two stages: (1)&#160;`libxs_mhd_read_header`, and (2)&#160;`libxs_mhd_read`. The latter allows to allocate a properly sized buffer, which is then used to take the data per `libxs_mhd_read`. When reading data, an on-the-fly type conversion is supported. Moreover, data already in memory can be compared against file-data without allocating memory or reading the file into memory.
