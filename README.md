...
The behavior of the intercepted GEMM routines (statically wrapped or via LD_PRELOAD) can be controlled using the environment variable LIBXS_MP i.e., 0:&#160;sequential below-threshold routine without OpenMP but with fallback to BLAS (default when only linking libxs), 1:&#160;OpenMP-parallelized but without internal parallel region, and 2:&#160;OpenMP-parallelized with internal parallel region (default when only linking libxsext). However, in case of the static wrapper, it is required to link against `libxsext` and `libxs`.
