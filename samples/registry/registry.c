...
{
  const int size = LIBXS_DEFAULT(1 << 20, 1 < argc ? atoi(argv[1]) : 0);
  unsigned long long start;
  double duration;
  int i;

  fprintf(stdout, "Dispatching %i calls...\n", size);
#if 0 != LIBXS_JIT
  ...
  /* first JIT attempt may actually generate code (which is here out of interest) */
