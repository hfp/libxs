...
#   pragma offload target(LIBXS_OFFLOAD_TARGET)
    ...
    if (EXIT_SUCCESS == result) {
      ...
      if (0 == libxs_gemm_diff_imci(&a.descriptor, &b.descriptor)) {
        fprintf(stderr, "using IMCI code path\n");
        result = 31;
      }
      else if (0 == libxs_gemm_diff_imci(&b.descriptor, &a.descriptor)) {
        fprintf(stderr, "using IMCI code path\n");
        result = 32;
      }
      else if (0 != libxs_gemm_diff_imci(&a.descriptor, &a.descriptor)) {
        fprintf(stderr, "using IMCI code path\n");
        result = 33;
      }
      else if (0 != libxs_gemm_diff_imci(&b.descriptor, &b.descriptor)) {
        fprintf(stderr, "using IMCI code path\n");
        result = 34;
      }
      else if (1 != libxs_gemm_diffn_imci(&a.descriptor, &local[0].desc, 0/*hint*/,
        sizeof(local) / sizeof(*local), sizeof(*local)))
      {
        fprintf(stderr, "using IMCI-based diffn-search\n");
        result = 35;
      }
      else if (6 != libxs_gemm_diffn_imci(&b.descriptor, &local[0].desc, 2/*hint*/,
        sizeof(local) / sizeof(*local), sizeof(*local)))
      {
        fprintf(stderr, "using IMCI-based diffn-search\n");
        result = 36;
