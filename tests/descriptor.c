...
int main()
{
  ...
  union { libxs_gemm_descriptor descriptor; char simd[32]; } a;
  libxs_gemm_descriptor b;
  ...
  for (i = LIBXS_GEMM_DESCRIPTOR_SIZE; i < sizeof(a.simd); ++i) a.simd[i] = 0;
#endif
  ...
  if (0 == libxs_gemm_diff(&a.descriptor, &b)) {
  ...
  else if (0 == libxs_gemm_diff(&b, &a.descriptor)) {
    fprintf(stderr, "using static code path\n");
    return 3;
  }
