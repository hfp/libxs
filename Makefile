...
INDICES ?= $(shell python $(SCRDIR)/libxs_utilities.py $(words $(M)) $(words $(N)) $(M) $(N) $(K))
...
$(SRCDIR)/%.c: $(HEADER)
	@python $(SCRDIR)/libxs_impl_mm.py $(ROW_MAJOR) $(ALIGNED_STORES) $(ALIGNED_LOADS) $(ALIGNMENT) -3 `echo $* | awk -F_ '{ print $$2" "$$3" "$$4 }'` > $@
