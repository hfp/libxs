...
ifeq ($(GENTARGET),noarch)
	@true || echo "#define LIBXS_GENTARGET_knl_dp" >> $@
	@if [[ 30 -ge $(NVALUE) ]]; then \
		true || echo "#define LIBXS_GENTARGET_knl_sp" >> $@; \
	fi
	...
	@true || echo "#define LIBXS_GENTARGET_wsm_dp" >> $@
	...
	@if [[ 30 -ge $(NVALUE) ]]; then \
		PS4=''; set -x; \
		#$(SCRDIR)/generator dense $@ libxs_s$(basename $(notdir $@))_hsw $(MVALUE2) $(NVALUE2) $(KVALUE) $(LDA) $(LDB) $(LDCSP) 0 $(ALIGNED_STORES) 1 knl nopf SP > /dev/null; \
	fi
else
