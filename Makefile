...
ifeq (noarch,$(GENTARGET))
	...
	@echo >> $@
	@echo >> $@
	$(BINDIR)/generator dense $@ libxs_d$(basename $(notdir $@))_knl $(MVALUE2) $(NVALUE2) $(KVALUE) $(LDA) $(LDB) $(LDCDP) 1 1 0 $(ALIGNED_ST) knl $(PREFETCH_SCHEME) DP
	$(BINDIR)/generator dense $@ libxs_s$(basename $(notdir $@))_knl $(MVALUE2) $(NVALUE2) $(KVALUE) $(LDA) $(LDB) $(LDCSP) 1 1 0 $(ALIGNED_ST) knl $(PREFETCH_SCHEME) SP
	$(BINDIR)/generator dense $@ libxs_d$(basename $(notdir $@))_hsw $(MVALUE2) $(NVALUE2) $(KVALUE) $(LDA) $(LDB) $(LDCDP) 1 1 0 $(ALIGNED_ST) hsw $(PREFETCH_SCHEME) DP
	$(BINDIR)/generator dense $@ libxs_s$(basename $(notdir $@))_hsw $(MVALUE2) $(NVALUE2) $(KVALUE) $(LDA) $(LDB) $(LDCSP) 1 1 0 $(ALIGNED_ST) hsw $(PREFETCH_SCHEME) SP
	$(BINDIR)/generator dense $@ libxs_d$(basename $(notdir $@))_snb $(MVALUE2) $(NVALUE2) $(KVALUE) $(LDA) $(LDB) $(LDCDP) 1 1 0 $(ALIGNED_ST) snb $(PREFETCH_SCHEME) DP
	$(BINDIR)/generator dense $@ libxs_s$(basename $(notdir $@))_snb $(MVALUE2) $(NVALUE2) $(KVALUE) $(LDA) $(LDB) $(LDCSP) 1 1 0 $(ALIGNED_ST) snb $(PREFETCH_SCHEME) SP
	$(BINDIR)/generator dense $@ libxs_d$(basename $(notdir $@))_wsm $(MVALUE2) $(NVALUE2) $(KVALUE) $(LDA) $(LDB) $(LDCDP) 1 1 0 $(ALIGNED_ST) wsm $(PREFETCH_SCHEME) DP
	$(BINDIR)/generator dense $@ libxs_s$(basename $(notdir $@))_wsm $(MVALUE2) $(NVALUE2) $(KVALUE) $(LDA) $(LDB) $(LDCSP) 1 1 0 $(ALIGNED_ST) wsm $(PREFETCH_SCHEME) SP
else
	@echo "#define LIBXS_GENTARGET_$(GENTARGET)_dp" >> $@
	@echo "#define LIBXS_GENTARGET_$(GENTARGET)_sp" >> $@
	@echo >> $@
	@echo >> $@
	$(BINDIR)/generator dense $@ libxs_d$(basename $(notdir $@))_$(GENTARGET) $(MVALUE2) $(NVALUE2) $(KVALUE) $(LDA) $(LDB) $(LDCDP) 1 1 0 $(ALIGNED_ST) $(GENTARGET) $(PREFETCH_SCHEME) DP
	$(BINDIR)/generator dense $@ libxs_s$(basename $(notdir $@))_$(GENTARGET) $(MVALUE2) $(NVALUE2) $(KVALUE) $(LDA) $(LDB) $(LDCSP) 1 1 0 $(ALIGNED_ST) $(GENTARGET) $(PREFETCH_SCHEME) SP
endif
	$(BINDIR)/generator dense $@ libxs_d$(basename $(notdir $@))_knc $(MVALUE2) $(NVALUE2) $(KVALUE) $(LDA) $(LDB) $(LDCDP) 1 1 0 $(ALIGNED_ST) knc $(PREFETCH_SCHEME) DP
	$(BINDIR)/generator dense $@ libxs_s$(basename $(notdir $@))_knc $(MVALUE2) $(NVALUE2) $(KVALUE) $(LDA) $(LDB) $(LDCSP) 1 1 0 $(ALIGNED_ST) knc $(PREFETCH_SCHEME) SP
	@sed -i'' \
...
$(BLDDIR)/libxs.c: $(INCDIR)/libxs.h $(SCRDIR)/libxs_dispatch.py
	@mkdir -p $(dir $@)
	@python $(SCRDIR)/libxs_dispatch.py $(THRESHOLD) $(SPARSITY) $(INDICES) > $@
...
ifneq ($(abspath $(OUTDIR)),$(abspath .))
	...
	@rm -f $(OUTDIR)/intel64/libxs.$(LIBEXT) $(OUTDIR)/mic/libxs.$(LIBEXT)
endif
else
	@rm -f $(OUTDIR)/intel64/libxs.$(LIBEXT) $(OUTDIR)/mic/libxs.$(LIBEXT)
endif
