...
ifneq ("$(M)$(N)$(K)","")
	INDICES ?= $(shell python $(SCRDIR)/libxs_utilities.py -2 $(words $(M)) $(words $(N)) $(M) $(N) $(K))
else
	INDICES ?= $(shell python $(SCRDIR)/libxs_utilities.py -1 '$(MNK)')
endif
...
header: $(HEADER)
$(HEADER): $(SRCDIR)/libxs.0.h $(SRCDIR)/libxs.1.h $(SRCDIR)/libxs.2.h
...
ifeq ($(GENASM),0)
	@python $(SCRDIR)/libxs_impl_mm.py $(ROW_MAJOR) $(ALIGNED_STORES) $(ALIGNED_LOADS) $(ALIGNMENT) -3 $(MVALUE) $(NVALUE) $(KVALUE) > $@
else
ifneq ($(ALIGNED_STORES),0) # aligned stores
ifneq ($(ROW_MAJOR),0) # row-major
	$(eval DPLDC := $(shell python $(SCRDIR)/libxs_utilities.py 8 $(NVALUE) $(ALIGNMENT) $(ALIGNED_STORES)))
	$(eval SPLDC := $(shell python $(SCRDIR)/libxs_utilities.py 4 $(NVALUE) $(ALIGNMENT) $(ALIGNED_STORES)))
else # column-major
	$(eval DPLDC := $(shell python $(SCRDIR)/libxs_utilities.py 8 $(MVALUE) $(ALIGNMENT) $(ALIGNED_STORES)))
	$(eval SPLDC := $(shell python $(SCRDIR)/libxs_utilities.py 4 $(MVALUE) $(ALIGNMENT) $(ALIGNED_STORES)))
endif
...
ifeq ($(GENTARGET),noarch)
	@if [[ 0 == $$(($(NVALUE) % 3)) ]]; then \
		PS4=''; set -x; \
		$(SCRDIR)/generator dense $@ libxs_d$(basename $(notdir $@))_wsm $(MVALUE) $(NVALUE) $(KVALUE) $(MVALUE) $(KVALUE) $(DPLDC) 1 wsm nopf DP; \
		$(SCRDIR)/generator dense $@ libxs_s$(basename $(notdir $@))_wsm $(MVALUE) $(NVALUE) $(KVALUE) $(MVALUE) $(KVALUE) $(SPLDC) 1 wsm nopf SP; \
		$(SCRDIR)/generator dense $@ libxs_d$(basename $(notdir $@))_snb $(MVALUE) $(NVALUE) $(KVALUE) $(MVALUE) $(KVALUE) $(DPLDC) 1 snb nopf DP; \
		$(SCRDIR)/generator dense $@ libxs_s$(basename $(notdir $@))_snb $(MVALUE) $(NVALUE) $(KVALUE) $(MVALUE) $(KVALUE) $(SPLDC) 1 snb nopf SP; \
		$(SCRDIR)/generator dense $@ libxs_d$(basename $(notdir $@))_hsw $(MVALUE) $(NVALUE) $(KVALUE) $(MVALUE) $(KVALUE) $(DPLDC) 1 hsw nopf DP; \
		$(SCRDIR)/generator dense $@ libxs_s$(basename $(notdir $@))_hsw $(MVALUE) $(NVALUE) $(KVALUE) $(MVALUE) $(KVALUE) $(SPLDC) 1 hsw nopf SP; \
	fi
else
	@if [[ 0 == $$(($(NVALUE) % 3)) ]]; then \
		PS4=''; set -x; \
		$(SCRDIR)/generator dense $@ libxs_d$(basename $(notdir $@))_$(GENTARGET) $(MVALUE) $(NVALUE) $(KVALUE) $(MVALUE) $(KVALUE) $(DPLDC) 1 $(GENTARGET) nopf DP; \
		$(SCRDIR)/generator dense $@ libxs_s$(basename $(notdir $@))_$(GENTARGET) $(MVALUE) $(NVALUE) $(KVALUE) $(MVALUE) $(KVALUE) $(SPLDC) 1 $(GENTARGET) nopf SP; \
	fi
endif
	@if [[ 30 -ge $(NVALUE) ]]; then \
		PS4=''; set -x; \
		$(SCRDIR)/generator dense $@ libxs_d$(basename $(notdir $@))_knc $(MVALUE) $(NVALUE) $(KVALUE) $(MVALUE) $(KVALUE) $(DPLDC) 1 knc nopf DP; \
		$(SCRDIR)/generator dense $@ libxs_s$(basename $(notdir $@))_knc $(MVALUE) $(NVALUE) $(KVALUE) $(MVALUE) $(KVALUE) $(SPLDC) 1 knc nopf SP; \
	fi
	@sed -i \
		...
		$@
	@python $(SCRDIR)/libxs_impl_mm.py $(ROW_MAJOR) $(ALIGNED_STORES) $(ALIGNED_LOADS) $(ALIGNMENT) 0 $(MVALUE) $(NVALUE) $(KVALUE) >> $@
endif
...
$(OBJDIR)/mic/%.o: $(SRCDIR)/%.c $(HEADER) $(SRCDIR)/libxs_isa.h
	@mkdir -p $(OBJDIR)/mic
	$(CC) $(CFLMIC) -I$(INCDIR) -c $< -o $@
$(OBJDIR)/mic/%.o: $(SRCDIR)/%.cpp $(HEADER) $(SRCDIR)/libxs_isa.h
	@mkdir -p $(OBJDIR)/mic
	$(CXX) $(CXXFLMIC) -I$(INCDIR) -c $< -o $@

compile_hst: $(OBJFILES_HST)
$(OBJDIR)/intel64/%.o: $(SRCDIR)/%.c $(HEADER) $(SRCDIR)/libxs_isa.h
	@mkdir -p $(OBJDIR)/intel64
	$(CC) $(CFLAGS) -I$(INCDIR) -c $< -o $@
$(OBJDIR)/intel64/%.o: $(SRCDIR)/%.cpp $(HEADER) $(SRCDIR)/libxs_isa.h
	@mkdir -p $(OBJDIR)/intel64
