...
SRCFILES_GEN_LIB = $(patsubst %,$(ROOTDIR)/$(SRCDIR)/%,$(wildcard $(ROOTDIR)/$(SRCDIR)/generator_*.c) libxs_trace.c libxs_generator.c)
SRCFILES_GEN_GEMM_BIN = $(patsubst %,$(ROOTDIR)/$(SRCDIR)/%,libxs_generator_gemm_driver.c)
SRCFILES_GEN_CONVWINO_BIN = $(patsubst %,$(ROOTDIR)/$(SRCDIR)/%,libxs_generator_convolution_winograd_driver.c)
SRCFILES_GEN_CONV_BIN = $(patsubst %,$(ROOTDIR)/$(SRCDIR)/%,libxs_generator_convolution_driver.c)
...
information = \
	...
	$(info A parallelized BLAS should be linked with LIBXS.), \
	$(NULL)))
...
.PHONY: samples
samples: lib_hst
	@find $(ROOTDIR)/$(SPLDIR) -type f -name Makefile | grep -v /gxm/ | grep -v /lstmcell/ | grep -v /rnndriver/ | grep -v /pyfr/ \
...
ifneq ($(abspath $(INSTALL_ROOT)),$(abspath .))
	...
	@ls -1 $(INCDIR)/libxs*.h | grep -v libxs_source.h | xargs -I {} $(CP) -v {} $(INSTALL_ROOT)/$(PINCDIR)
	...
	@$(ROOTDIR)/$(SCRDIR)/libxs_source.sh $(patsubst $(PINCDIR)/%,%,$(PSRCDIR)) \
		> $(INSTALL_ROOT)/$(PINCDIR)/libxs_source.h
