...
OBJFILES_HST = $(patsubst %,$(BLDDIR)/intel64/mm_%.o,$(INDICES)) $(BLDDIR)/intel64/libxs_crc32.o $(BLDDIR)/intel64/libxs_dispatch.o $(BLDDIR)/intel64/libxs.o
OBJFILES_MIC = $(patsubst %,$(BLDDIR)/mic/mm_%.o,$(INDICES)) $(BLDDIR)/mic/libxs_crc32.o $(BLDDIR)/mic/libxs_dispatch.o $(BLDDIR)/mic/libxs.o
...
$(INCDIR)/libxs.f90: $(ROOTDIR)/Makefile $(SCRDIR)/libxs_interface.py $(SCRDIR)/libxs_utilities.py $(SRCDIR)/libxs.template.f90
	@mkdir -p $(dir $@)
	@python $(SCRDIR)/libxs_interface.py $(SRCDIR)/libxs.template.f90 $(ROW_MAJOR) $(ALIGNMENT) $(ALIGNED_ST) $(ALIGNED_LD) \
		$(PREFETCH_TYPE) $(JIT) $(shell echo $$((0<$(THRESHOLD)?$(THRESHOLD):0))) $(BETA) $(INDICES) > $@
ifeq (0,$(OFFLOAD))
	@TMPFILE=`mktemp`
...
$(BLDDIR)/intel64/%.o: $(SRCDIR)/%.c $(INCDIR)/libxs.h
	@mkdir -p $(dir $@)
	$(CC) $(CFLAGS) $(DFLAGS) $(IFLAGS) $(TARGET) -c $< -o $@
...
$(OUTDIR)/intel64/libxs.$(LIBEXT): $(OBJFILES_HST) $(OBJFILES_GEN_LIB) $(BLDDIR)/intel64/libxs_build_jit.o
	@mkdir -p $(dir $@)
ifeq (0,$(STATIC))
	$(LD) -o $@ $^ -shared $(LDFLAGS) $(CLDFLAGS)
else
