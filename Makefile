...
ifneq ($(ROW_MAJOR),0) # row-major
	...
	$(SCRDIR)/generator dense $@ libxs_d$(basename $(notdir $@)) $(MVALUE) $(NVALUE) $(KVALUE) $(MVALUE) $(NVALUE) $(DPLDC) 1 $(GENTARGET) pfsigonly DP
	@sed -i \
	...
	$(SCRDIR)/generator dense $@ libxs_s$(basename $(notdir $@)) $(MVALUE) $(NVALUE) $(KVALUE) $(MVALUE) $(NVALUE) $(SPLDC) 1 $(GENTARGET) pfsigonly SP
	@sed -i \
...
main: $(MAIN)
$(MAIN): $(HEADER)
	@python $(SCRDIR)/libxs_dispatch.py $@ $(words $(M)) $(words $(N)) $(M) $(N) $(K) > $@

compile_mic: $(OBJFILES_MIC)
$(OBJDIR)/mic/%.o: $(SRCDIR)/%.c $(SRCDIR)/libxs_isa.h $(HEADER)
	@mkdir -p $(OBJDIR)/mic
	$(CC) $(CFLMIC) -I$(INCDIR) -c $< -o $@
$(OBJDIR)/mic/%.o: $(SRCDIR)/%.cpp $(SRCDIR)/libxs_isa.h $(HEADER)
...
$(OBJDIR)/intel64/%.o: $(SRCDIR)/%.cpp $(SRCDIR)/libxs_isa.h $(HEADER)
	@mkdir -p $(OBJDIR)/intel64
