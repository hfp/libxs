...
INDICES ?= $(shell python $(SCRDIR)/libxs_utilities.py 0 $(words $(M)) $(words $(N)) $(M) $(N) $(K))
...
main: $(MAIN)
$(MAIN): $(HEADER)
	@python $(SCRDIR)/libxs_dispatch.py $(THRESHOLD) $(SPARSITY) $(words $(M)) $(words $(N)) $(M) $(N) $(K) > $@
