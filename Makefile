...
ifeq ($(GENASM),0)
	...
	@if [[ ( 0 == $$(($(NVALUE) % 3)) && "knl" != $(GENTARGET) ) || ( 30 -ge $(NVALUE) && "knl" == $(GENTARGET) ) ]]; then echo "#define LIBXS_GENTARGET_$(GENTARGET)" >> $@; fi
...
ifeq ($(GENTARGET),noarch)
	@if [[ 0 == $$(($(NVALUE) % 3)) ]]; then \
		...
		false && $(SCRDIR)/generator dense $@ libxs_d$(basename $(notdir $@))_hsw $(MVALUE2) $(NVALUE2) $(KVALUE) $(LDA) $(LDB) $(LDCDP) 0 $(ALIGNED_STORES) 1 knl nopf DP; \
		false && $(SCRDIR)/generator dense $@ libxs_s$(basename $(notdir $@))_hsw $(MVALUE2) $(NVALUE2) $(KVALUE) $(LDA) $(LDB) $(LDCSP) 0 $(ALIGNED_STORES) 1 knl nopf SP; \
	fi
else
