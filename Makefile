...
ifneq ($(ALIGNED_STORES),0) # aligned stores
	...
	$(eval LDB := $(KVALUE))
ifeq ($(GENASM),0)
	@python $(SCRDIR)/libxs_impl_mm.py $(ROW_MAJOR) $(ALIGNED_STORES) $(ALIGNED_LOADS) $(ALIGNMENT) -4 $(MVALUE) $(NVALUE) $(KVALUE) > $@
else
...
ifeq ($(GENTARGET),noarch)
	...
	@sed -i'' \
		...
		$@
	@python $(SCRDIR)/libxs_impl_mm.py $(ROW_MAJOR) $(ALIGNED_STORES) $(ALIGNED_LOADS) $(ALIGNMENT) -1 $(MVALUE) $(NVALUE) $(KVALUE) >> $@
endif
