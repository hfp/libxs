...
# a jit enanled version of LIBXS
...
ifneq ("$(M)$(N)$(K)","")
	INDICES ?= $(shell python $(SCRDIR)/libxs_utilities.py -2 $(THRESHOLD) $(words $(M)) $(words $(N)) $(M) $(N) $(K))
else
	INDICES ?= $(shell python $(SCRDIR)/libxs_utilities.py -1 $(THRESHOLD) '$(MNK)')
endif
...
ifneq (nopf,$(PREFETCH_SCHEME))
	SUPPRESS_UNUSED_VARIABLE_WARNINGS += LIBXS_UNUSED(A_prefetch); LIBXS_UNUSED(B_prefetch); LIBXS_UNUSED(C_prefetch);
endif
...
ifeq (0,$(STATIC))
	...
	@rm -rf $(ROOTDIR)/include/libxs_generator.h
	@cat $(SRCDIR)/generator_extern_typedefs.h >> $(ROOTDIR)/include/libxs_generator.h
	@cat $(SRCDIR)/generator_dense.h | grep -v "generator_extern_typedefs.h" >> $(ROOTDIR)/include/libxs_generator.h
	@cat $(SRCDIR)/generator_sparse.h | grep -v "generator_extern_typedefs.h" >> $(ROOTDIR)/include/libxs_generator.h
...
$(BINDIR)/generator: $(OBJFILES_GEN_BIN) $(OUTDIR)/intel64/libxsgen.$(LIBEXT) $(ROOTDIR)/Makefile
	@mkdir -p $(dir $@)
...
ifneq (0,$(ALIGNED_STORES)) # aligned stores
	$(eval LDCDP := $(shell python $(SCRDIR)/libxs_utilities.py 8 $(MVALUE2) $(ALIGNED_ST)))
	$(eval LDCSP := $(shell python $(SCRDIR)/libxs_utilities.py 4 $(MVALUE2) $(ALIGNED_ST)))
...
ifeq (noarch,$(GENTARGET))
	...
	@echo >> $@
	@echo >> $@
	$(BINDIR)/generator dense $@ libxs_d$(basename $(notdir $@))_knl $(MVALUE2) $(NVALUE2) $(KVALUE) $(LDA) $(LDB) $(LDCDP) 1 $(BETA) 0 $(ALIGNED_ST) knl $(PREFETCH_SCHEME) DP
	$(BINDIR)/generator dense $@ libxs_s$(basename $(notdir $@))_knl $(MVALUE2) $(NVALUE2) $(KVALUE) $(LDA) $(LDB) $(LDCSP) 1 $(BETA) 0 $(ALIGNED_ST) knl $(PREFETCH_SCHEME) SP
	$(BINDIR)/generator dense $@ libxs_d$(basename $(notdir $@))_hsw $(MVALUE2) $(NVALUE2) $(KVALUE) $(LDA) $(LDB) $(LDCDP) 1 $(BETA) 0 $(ALIGNED_ST) hsw $(PREFETCH_SCHEME) DP
	$(BINDIR)/generator dense $@ libxs_s$(basename $(notdir $@))_hsw $(MVALUE2) $(NVALUE2) $(KVALUE) $(LDA) $(LDB) $(LDCSP) 1 $(BETA) 0 $(ALIGNED_ST) hsw $(PREFETCH_SCHEME) SP
	$(BINDIR)/generator dense $@ libxs_d$(basename $(notdir $@))_snb $(MVALUE2) $(NVALUE2) $(KVALUE) $(LDA) $(LDB) $(LDCDP) 1 $(BETA) 0 $(ALIGNED_ST) snb $(PREFETCH_SCHEME) DP
	$(BINDIR)/generator dense $@ libxs_s$(basename $(notdir $@))_snb $(MVALUE2) $(NVALUE2) $(KVALUE) $(LDA) $(LDB) $(LDCSP) 1 $(BETA) 0 $(ALIGNED_ST) snb $(PREFETCH_SCHEME) SP
	$(BINDIR)/generator dense $@ libxs_d$(basename $(notdir $@))_wsm $(MVALUE2) $(NVALUE2) $(KVALUE) $(LDA) $(LDB) $(LDCDP) 1 $(BETA) 0 $(ALIGNED_ST) wsm $(PREFETCH_SCHEME) DP
	$(BINDIR)/generator dense $@ libxs_s$(basename $(notdir $@))_wsm $(MVALUE2) $(NVALUE2) $(KVALUE) $(LDA) $(LDB) $(LDCSP) 1 $(BETA) 0 $(ALIGNED_ST) wsm $(PREFETCH_SCHEME) SP
else
...
.PHONY: main
main: $(BLDDIR)/libxs.c
$(BLDDIR)/libxs.c: $(INCDIR)/libxs.h $(SCRDIR)/libxs_dispatch.py
	@mkdir -p $(dir $@)
	@python $(SCRDIR)/libxs_dispatch.py $(THRESHOLD) $(SPARSITY) $(JIT) $(INDICES) > $@
ifneq (0,$(JIT))
	@cat $(SRCDIR)/libxs_jit_builder.template.c >> $@
	@rm -rf $(ROOTDIR)/include/libxs_generator.h
	@cat $(SRCDIR)/generator_extern_typedefs.h >> $(ROOTDIR)/include/libxs_generator.h
	@cat $(SRCDIR)/generator_dense.h | grep -v "generator_extern_typedefs.h" >> $(ROOTDIR)/include/libxs_generator.h
	@cat $(SRCDIR)/generator_sparse.h | grep -v "generator_extern_typedefs.h" >> $(ROOTDIR)/include/libxs_generator.h
...
$(OUTDIR)/mic/libxs.$(LIBEXT): $(OBJFILES_MIC) $(BLDDIR)/mic/libxs.o
else
...
ifeq (undefined,$(origin NO_MAIN))
ifneq (0,$(JIT))
$(OUTDIR)/intel64/libxs.$(LIBEXT): $(OBJFILES_HST) $(OBJFILES_GEN_LIB) $(BLDDIR)/intel64/libxs.o
else
$(OUTDIR)/intel64/libxs.$(LIBEXT): $(OBJFILES_HST) $(BLDDIR)/intel64/libxs.o
endif
else
ifneq (0,$(JIT))
$(OUTDIR)/intel64/libxs.$(LIBEXT): $(OBJFILES_HST) $(OBJFILES_GEN_LIB)
else
$(OUTDIR)/intel64/libxs.$(LIBEXT): $(OBJFILES_HST)
endif
endif
	@mkdir -p $(dir $@)
ifeq (0,$(STATIC))
...
ifneq ($(abspath $(BLDDIR)),$(abspath .))
	...
	@rm -f $(OBJECTS) $(BLDDIR)/libxs.c $(BLDDIR)/*.mod
endif
else
	@rm -f $(OBJECTS) $(BLDDIR)/libxs.c $(BLDDIR)/*.mod
endif
