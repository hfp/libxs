...
VERSION_API ?= $(shell $(SCRDIR)/libxs_utilities.py 0 $(VERSION_MAJOR).$(VERSION_MINOR).$(VERSION_UPDATE))
...
deb:
	@if [ "" != "$$(which git)" ]; then \
		...
		VERSION_ARCHIVE_SONAME=$$($(SCRDIR)/libxs_utilities.py 0 $${VERSION_ARCHIVE}); \
	fi; \
