...
ifeq ($(GENTARGET),noarch)
	@if [[ 0 == $$(($(NVALUE) % 3)) ]]; then \
		...
		#$(SCRDIR)/generator dense $@ libxs_d$(basename $(notdir $@))_hsw $(MVALUE2) $(NVALUE2) $(KVALUE) $(LDA) $(LDB) $(LDCDP) 0 $(ALIGNED_STORES) 1 knl nopf DP; \
		#$(SCRDIR)/generator dense $@ libxs_s$(basename $(notdir $@))_hsw $(MVALUE2) $(NVALUE2) $(KVALUE) $(LDA) $(LDB) $(LDCSP) 0 $(ALIGNED_STORES) 1 knl nopf SP; \
	fi
else
