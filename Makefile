...
LIB_KNC  ?= $(LIBDIR)/mic/libxs.a
LIB_HST  ?= $(LIBDIR)/intel64/libxs.a
INC_KNC   = $(INCDIR)/libxs.h
MAIN  = $(SRCDIR)/libxs.c
...
header_knc: $(INC_KNC)
$(INC_KNC): $(INCDIR)/libxs.0 $(INCDIR)/libxs.1 $(INCDIR)/libxs.2
	@cat $(INCDIR)/libxs.0 > $@
	...
	@cat $(INCDIR)/libxs.1 >> $@
	@echo >> $@
	@python $(SCRDIR)/libxs_interface.py $(ROW_MAJOR) $(words $(INDICES_M)) $(words $(INDICES_N)) $(INDICES_M) $(INDICES_N) $(INDICES_K) >> $@
	@cat $(INCDIR)/libxs.2 >> $@
